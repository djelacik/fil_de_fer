...
/*   Updated: 2024/10/16 12:12:35 by djelacik         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/fdf.h"

static t_color	interpolate_color(t_color start_clr,
	t_color end_clr, double fraction)
{
	t_color		result;
...
t_color	color_on_height(double z, t_map *map)
{
	...
	color.r = (int)(50 * (1 - ratio) + 255 * ratio);
	color.g = (int)(0 * (1 - ratio) + 100 * ratio);
	...
	while (y < map->height)
	{
		...
		while (x < map->width)
		{
			if (map->has_any_color)
			{
				...
				map->map[y][x].color = color_on_height(map->map[y][x].z, map);
			x++;
		}
		y++;
	}
